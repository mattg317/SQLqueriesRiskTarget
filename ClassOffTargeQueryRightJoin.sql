SELECT * FROM
(
SELECT risk.[University Name], risk.[Class Section], risk.[Start Date], risk.avg_pace, risk.avg_support,
 risk.avg_selfmast, risk.avg_engage, risk.avg_clarity, risk.avg_knowledge, risk.avg_lowsupport
FROM (	
	SELECT 
		feedback.[University Name],
		feedback.[Class Section],
		feedback.[Start Date], 
	...
	GROUP BY feedback.[University Name], feedback.[Class Section], feedback.[Start Date]
)risk
WHERE risk.[Class Section] <> 'TRI-TEST-FSF'
	AND risk.avg_pace <4.5 
	AND risk.avg_support >3.75 
	AND risk.avg_selfmast>3 
	AND risk.avg_engage > 3.75 
	AND risk.avg_clarity > 3.75
	AND risk.avg_knowledge > 3.75
	AND risk.avg_lowsupport <= 0.1
	AND ((risk.avg_pace > 4.25 AND risk.avg_pace < 4.5) 
	OR (risk.avg_support >3 AND risk.avg_support<4)
	OR (risk.avg_selfmast>3 AND risk.avg_selfmast <3.25)
	OR (risk.avg_engage > 3.5 AND risk.avg_engage < 4) 
	OR (risk.avg_clarity > 3.5 AND risk.avg_clarity <4)
	OR (risk.avg_knowledge > 3.5 AND risk.avg_knowledge <4)
	OR (risk.avg_lowsupport > 0.05 AND risk.avg_lowsupport < 0.095))
 
 
)timeTable
WHERE timeTable.[Start Date] BETWEEN AddMonths(Now(), -1) AND NOW()
